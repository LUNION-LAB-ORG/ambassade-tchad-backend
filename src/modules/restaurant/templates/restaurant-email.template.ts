    ...
    NEW_RESTAURANT: EmailTemplate<
        {
            ...
            subject: (ctx) => `🎉 Nouveau restaurant : ${ctx.data.restaurant.name} a rejoint Ambassade Tchad !`,
            content: (ctx) => {
    ...
    WELCOME_RESTAURANT: EmailTemplate<
        {
            ...
            subject: (ctx) => `🎉 Bienvenue à ${ctx.data.restaurant.name} dans la famille Ambassade Tchad !`,
            content: (ctx) => {
                const restaurantDashboardUrl = this.configService.get<string>('FRONTEND_URL') || "";
                const supportUrl = this.configService.get<string>('SUPPORT_URL') || `mailto:${this.configService.get<string>('AMBASSADE_TCHAD_SUPPORT')}` || "";
                ...
                const emailContent = [
                    ...
                    this.emailComponentsService.Message(
                        `Toute l'équipe de Ambassade Tchad vous souhaite la bienvenue ! Votre restaurant ${ctx.data.restaurant.name} est maintenant officiellement en ligne. Nous sommes impatients de voir vos plats régaler nos clients.`
                    ),
