...
export class UsersService {
     ...
     */
    async create(req: Request, createUserDto: CreateUserDto): Promise<Omit<User, 'password'> & {
     ...
     */
    async demandeurDetail(userId: string) {
    ...
    /**
     * Met à jour le mot de passe d'un utilisateur demandeur.
     * @param req L'objet requête (l'acteur de l'action).
     * @param id L'ID de l'utilisateur demandeur dont le mot de passe doit être mis à jour.
     * @param updateUserPasswordDto
     * @returns L'utilisateur mis à jour (sans le mot de passe).
     * @throws BadRequestException si l'utilisateur n'est pas trouvé, si le mot de passe actuel est invalide, ou si l'utilisateur tente de changer son propre mot de passe.
     */
    async updateDemandeurPassword(req: Request, id: string, updateUserPasswordDto: UpdateUserPasswordDto) {
        const user = req.user as User;

        // Si les données sont dans req.body.data, les extraire
        let passwordData = updateUserPasswordDto;
        if (!updateUserPasswordDto.password && req.body) {
            passwordData = req.body as UpdateUserPasswordDto;
        }
        ...
        const isPasswordValid = await bcrypt.compare(passwordData.password, userToUpdate.password);
        if (!isPasswordValid) {
            throw new BadRequestException('Mot de passe actuel invalide.');
        }

        // Vérifier que le nouveau mot de passe est bien défini
        ...
        const updatedUser = await this.prisma.user.update({
            where: {id: id},
            data: {password: hashedPassword, isPasswordChangeRequired: false},
            select: {
                id: true,
                email: true,
                firstName: true,
